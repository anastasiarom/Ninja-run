...
{
    Music music;
    music.openFromFile("files/theme.ogg");
    music.setLoop(true);
    if(W.sound)
        music.play();
    ...
    while (window.isOpen())
    {
        ...
        if (W.sound == false && music.getStatus() == SoundSource::Status::Playing&& W.currentWindow == "setup")
        {
            music.pause();
            //W.sound = true;
        }
        else
            if (W.sound == true && music.getStatus() == SoundSource::Status::Paused&&W.currentWindow == "setup")
            {
                music.play();
                //W.sound = false;
            }
...
bool StartGame(int l,RenderWindow& window, Windows &W )
{
    ...
    switch (l)
    {
    case 1: {lvl.LoadFromFile("files/level1.tmx", "files/Tileset.png"); break; }
    case 2: {lvl.LoadFromFile("files/level2.tmx", "files/Tileset2.png"); break; }
    case 3: {lvl.LoadFromFile("files/level3.tmx", "files/Tileset3.png"); break; }
    }
    ...
    while (window.isOpen())
    {
        ...
        for (it = entities.begin(); it != entities.end();)
        {
            Entity* e = *it;
            e->update(time);
            if (e->getLife() == false) { it = entities.erase(it); delete e; }
            else it++;
        }
        ...
        for (it = entities.begin(); it != entities.end(); it++)
        {
            ...
            if ((*it)->getName() == "Coin")
            {
                ...
                if (Ninja.getRect().intersects(coin->getRect()))
                {
                    ...
                    coin->setHealth(0);
                    if(W.sound)
                        Ninja.soundCoin.play();
                }
            }
        }

        view.setCenter(Ninja.getX(), 540);
        window.setView(view);
